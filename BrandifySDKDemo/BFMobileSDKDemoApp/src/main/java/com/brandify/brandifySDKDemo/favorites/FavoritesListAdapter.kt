...
class FavoritesListAdapter(private var parentActivity: FavoritesActivity, var favoritesList: BFListOfLocations?) : ArrayAdapter<BFLocation?>(parentActivity.applicationContext, -1) {
    ...
    override fun getView(pos: Int, convertViewParam: View?, parent: ViewGroup): View? {
        ...
        if (convertView == null) {
            ...
            holder.addressText = convertView.findViewById<View>(R.id.favorites_list_address) as TextView
        ...
        holder.pinImage?.setImageBitmap(BFUtils.writeTextOnDrawable(parentActivity.applicationContext, R.drawable.green_pin, (location.sequence + 1).toString()))
        ...
        if (location.state != null) {
            (holder.cityText?.text.toString() + ", " + location.state).also { holder.cityText?.text = it }
        }
        if (location.postalCode != null) {
            (holder.cityText?.text.toString() + " " + location.postalCode).also { holder.cityText?.text = it }
        }
